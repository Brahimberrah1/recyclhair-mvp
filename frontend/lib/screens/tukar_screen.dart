...
class TukarScreenState extends State<TukarScreen> {
  ...
  void validationCoin(int i) {
    if (jmlKoinDriver < point) {
      const snackBar = SnackBar(
        content: Text('Your coins are insufficient'),
      );
  ...
  void alert() {
    final alertTitles = ["Confirm"];
    final alertDetailPesanan =
        "Are you sure you want to exchange with a total of " +
            point.toString() +
            " coins?";
    DLAlert(
        cancelTitle: 'Cancel',
        alertTitle: 'Exchange Confirmation',
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: const Text(
              "Your coins have been successfully exchanged.",
              textAlign: TextAlign.center,
            ),
          );
        });
